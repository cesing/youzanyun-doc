...
deprecated: false
since: "2019-05-20"
detailUrl: "https://gateway.qima-inc.com/api-manager/detail?id=358"
---
...
| 参数名 | 类型 | 必填 | 示例 | 说明 |
|---|---|---|---|---|
| `micropage_update_request` | `object` | ✅ 是 | `` |  |
| `page_id` | `integer` | ✅ 是 | `` | 页面id |
| `user_id` | `integer` | ✅ 是 | `` | 用户id |
| `inner_item_id` | `integer` | ❌ 否 | `` | 内购项id（备用字段，不填） |
| `out_item_id` | `string` | ✅ 是 | `` | （内购项）外部商品编码 |
| `page_name` | `string` | ❌ 否 | `` | 页面名称 |
| `page_desc` | `string` | ❌ 否 | `` | 页面描述 |
| `view_url` | `string` | ❌ 否 | `` | 预览地址 |
| `re_edit_url` | `string` | ❌ 否 | `` | 重编辑地址 |
...
# 文档地址: https://gateway.qima-inc.com/api-manager/detail?id=358

curl -X POST 'https://open.youzanyun.com/api/youzan.skinfo/1.0.0' \
  -H 'Authorization: Bearer <YOUR_ACCESS_TOKEN>' \
  -H 'Content-Type: application/json' \
  -d '{
  "micropage_update_request": "<micropage_update_request>",
  "page_id": "<page_id>",
  "user_id": "<user_id>",
...
| 方法名称 | `updateMicroPage` |
| 超时时间 | 5000ms |
---
## 8. 关联接口
*（暂无关联数据，文档完善后将补充相关接口）*
---
_本文档由 AI 自动生成，源数据来自 [有赞云开放平台详情页](https://gateway.qima-inc.com/api-manager/detail?id=358)_